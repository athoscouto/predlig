...
for metric in metrics:
    ...
    for classifier, classifier_params in classifiers.items():
        ...
        message += "%s;%f/%s;%f/%s;%f/%s\n" % (classifier, FFS.best_score, len(FFS.best_var_subset), BaFE.best_score, len(BaFE.best_var_subset), BiFE.best_score, len(BiFE.best_var_subset))
        message += "%s;%f/%s;%f/%s;%f/%s\n" % (classifier, FFS.best_score, len(FFS.best_var_subset), BaFE.best_score, len(BaFE.best_var_subset), BiFE.best_score, len(BiFE.best_var_subset))
...
for metric in metrics:
    ...
    for classifier, classifier_params in classifiers.items():
        ...
        message += ";%s/%s" % (FS1.best_solution.fitness.values[0], len([i for i in range(len(FS1.best_solution)) if FS1.best_solution[i] == 1]))
        message += ";%s/%s" % (FS2.best_solution.fitness.values[0], len([i for i in range(len(FS2.best_solution)) if FS2.best_solution[i] == 1]))
        message += ";%s/%s" % (FS3.best_solution.fitness.values[0], len([i for i in range(len(FS3.best_solution)) if FS3.best_solution[i] == 1]))
        message += ";%s/%s\n" % (FS4.best_solution.fitness.values[0], len([i for i in range(len(FS4.best_solution)) if FS4.best_solution[i] == 1]))
