...
class BackwardFeatureElimination(WrapperFeatureSelection):
    ...
    def first_solutions_generation(self):
        return (self.best_temp_score is None)
    ...
    def reached_stopping_criteria(self):
        return ((self.best_score is not None and self.best_temp_score < self.best_score) or len(self.variable_subset) <= 1)
    ...
    def evaluate_possible_solutions(self):
        for variable_subset in self.possible_solutions:
            ...
            if self.best_temp_score is None or score >= self.best_temp_score:
...
class ForwardFeatureSelection(WrapperFeatureSelection):
    ...
    def reached_stopping_criteria(self):
        return ((self.best_score is not None and (self.best_temp_score < self.best_score or len(self.best_temp_var_subset) > len(self.best_var_subset))) or len(self.best_var_subset) >= self.number_variables)
    ...
    def evaluate_possible_solutions(self):
        ...
        if self.best_score is None or self.best_score < self.best_temp_score:
            self.best_score = self.best_temp_score
            self.best_var_subset = list(self.best_temp_var_subset)
...
class BidirectionalFeatureSelection(WrapperFeatureSelection):
    ...
    def first_solutions_generation(self):
        return (self.best_score is None)
    ...
    def reached_stopping_criteria(self):
        return ((self.best_score is not None and (self.best_temp_score < self.best_score or len(self.best_temp_var_subset) > len(self.best_var_subset))) or len(self.variable_subset) <= 1)
