...
class FeatureConstructor(object):
    ...
    def __init__(self, graph, node_1=None, node_2=None):
        ...
        if self.node_1 is not None and self.node_2 is not None:
    ...
    def katz_measure(self, beta=0.85, min_length=3):
        ...
        if nx.shortest_path_length(self.graph, self.node_1, self.node_2) >= min_length:
            for i in nx.all_simple_paths(self.graph, self.node_1, self.node_2, min_length):
                # TODO Athos Couto - [07-12-2015]: Check with Antonio if operator precedence is correct
    ...
    def betweenness_centrality(self):
        if (self.betweenness_dict is None):
    ...
    def average_neighbor_degree_sum(self):
        if (self.average_neighbor_degree_dict is None):
    ...
    def load_centrality_sum(self):
        if (self.load_centrality_dict is None):
    ...
    def communicability_centrality_sum(self):
        if (self.communicability_centrality_dict is None):
    ...
    def eigenvector_centrality_sum(self):
        if (self.eigenvector_centrality_dict is None):
    ...
    def closeness_vitality_sum(self):
        if (self.closeness_vitality_dict is None):
    ...
    def core_number_sum(self):
        if (self.core_number_dict is None):
